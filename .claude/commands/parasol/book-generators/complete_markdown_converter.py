#!/usr/bin/env python3
"""
Complete markdown to HTML converter with proper table column handling and Mermaid support
"""

import re
from pathlib import Path
from typing import List, Tuple, Dict
from mermaid_flow_converter import MermaidFlowConverter


class CompleteMarkdownConverter:
    """Complete markdown to HTML converter with table column fixes"""
    
    def __init__(self):
        self.code_blocks = []
        self.inline_codes = []
        self.mermaid_converter = MermaidFlowConverter()
    
    def convert(self, markdown: str) -> str:
        """Convert markdown to HTML with complete support"""
        html = markdown
        
        # 1. Convert flow diagrams to Mermaid first
        html = self._convert_flow_diagrams(html)
        
        # 2. Protect code blocks (including Mermaid blocks)
        html = self._protect_code_blocks(html)
        
        # 3. Convert tables BEFORE inline code to prevent table cells from being wrapped
        html = self._convert_tables(html)
        
        # 4. Process blockquotes with ALL nested content
        html = self._convert_blockquotes_complete(html)
        
        # 5. Now protect inline code after tables are converted
        html = self._protect_inline_code(html)
        
        # 6. Convert other block elements
        html = self._convert_headers(html)
        html = self._convert_lists(html)
        html = self._convert_horizontal_rules(html)
        
        # 7. Convert inline elements
    ...
    def _convert_flow_diagrams(self, text: str) -> str:
        """Convert flow diagrams to Mermaid format"""
        return self.mermaid_converter.convert_document(text)
            ...
            if lines[i].startswith(('    ', '\t')):
                ...
                if i > 0 and not lines[i-1].strip() and not lines[i-1].startswith('>'):
                    ...
                    if code_lines:
                        ...
                        result.append(f'CODEBLOCK{len(self.code_blocks) - 1}')
                        continue
    ...
    def _protect_inline_code(self, text: str) -> str:
        ...
        def save_inline(match):
            ...
            return f'INLINECODE{len(self.inline_codes) - 1}'
    ...
    def _convert_tables(self, text: str) -> str:
        ...
        while i < len(lines):
            ...
            if 'CODEBLOCK' in lines[i] or 'INLINECODE' in lines[i] or lines[i].startswith('<blockquote'):
                result.append(lines[i])
                i += 1
                continue
    ...
    def _build_html_table(self, lines: List[str]) -> str:
        """Build HTML table from markdown table lines with proper column handling"""
        if len(lines) < 2:
            return '\n'.join(lines)
        ...
        for line in lines:
            ...
            # Remove empty cells at start and end (from | at line boundaries)
        ...
        # Parse alignment from separator line (second row)
        alignments = []
        if len(all_rows) > 1:
            separator_cells = all_rows[1]
    ...
    def _convert_emphasis_safe(self, text: str) -> str:
        ...
        placeholder_pattern = r'((?:CODEBLOCK|INLINECODE)\d+)'
