#!/usr/bin/env python3
"""
Perfect ultimate markdown to HTML converter with fixed code block handling
"""
...
class PerfectUltimateMarkdownConverter:
    """Perfect ultimate markdown to HTML converter"""
    ...
    def _protect_code_blocks(self, text: str) -> str:
        ...
        # Fenced code blocks - Fixed regex to handle ```\n...\n``` and ```\n...```
        def save_fenced(match):
            lang = match.group(1) or ''
            code = match.group(2).rstrip()  # Remove trailing whitespace/newlines
    ...
    def _convert_tables(self, text: str) -> str:
        ...
        while i < len(lines):
            ...
            if '___CODEBLOCK_' in lines[i] or '___INLINECODE_' in lines[i]:
                result.append(lines[i])
                i += 1
                continue
