#!/usr/bin/env python3
"""Ultimate HTML generator with fixed code block handling"""
...
def process_code_blocks(text):
    ...
    # Process fenced code blocks first (with or without language specifier)
    ...
    # Handle various fenced code block formats
    text = re.sub(r'```(?:\w+)?\n(.*?)\n```', save_code_block, text, flags=re.DOTALL)
    text = re.sub(r'```(?:\w+)?\s*\n(.*?)\s*```', save_code_block, text, flags=re.DOTALL)
    ...
    while i < len(lines):
        ...
        if (lines[i].startswith('    ') or lines[i].startswith('\t')) and i > 0 and lines[i-1].strip() == '':
            # This might be a code block - collect consecutive indented lines
            code_lines = []
            start_i = i
            ...
            # Check if this looks like a code block (multiple lines or has code-like content)
            is_code_block = False
            if len(code_lines) > 2:
                is_code_block = True
            elif any(line for line in code_lines if '=' in line or '->' in line or '{' in line or '}' in line or '#' in line):
                is_code_block = True
            ...
            if is_code_block and code_lines:
                ...
                # Not a code block, restore the lines
                i = start_i
    ...
    # Also handle inline code blocks that might look like display blocks
    # This catches cases where code is marked with special characters
    code_block_pattern = r'^((?:[ ]{0,3}[\*\-]{3,}|={3,})\s*\n)(.*?)\n((?:[ ]{0,3}[\*\-]{3,}|={3,})\s*)$'
    
    def save_delimited_code_block(match):
        code = match.group(2)
        formatted_code = f'<pre><code>{escape_html(code)}</code></pre>'
        code_blocks.append(formatted_code)
        return placeholder_template.format(len(code_blocks) - 1)
    
    text = re.sub(code_block_pattern, save_delimited_code_block, text, flags=re.MULTILINE | re.DOTALL)
