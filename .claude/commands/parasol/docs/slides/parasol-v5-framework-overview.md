# Parasol V5.4 フレームワーク概要

> **価値駆動設計フレームワーク** - ビジネス価値からソフトウェア実装まで一貫した設計を実現
> **V5.4**: Axiomatic Design理論を統合し、設計の独立性と情報量最小化を保証
...
- **共通言語の確立** - ビジネスとITの架け橋
- **設計の独立性保証** - Axiomatic Designによる結合度最小化
- **情報量最小化** - 成功確率の最大化
...
| Phase | 名称 | 目的 | 主な成果物 | 書籍章 |
|-------|------|------|-----------|-------|
| 0 | 事前準備 | 基本情報収集 | 企業分析、スコープ定義 | Ch.6 |
| 1 | 敷地調査 | 制約と現状把握 | ステークホルダー分析、制約条件 | Ch.7 |
| 2 | コンセプト設計 | 価値の可視化 | Value Stage定義（VS0-VS7） | Ch.9-12 |
| 3 | 間取り設計 | 能力の体系化 | CL1/CL2/CL3定義 | Ch.13-16 |
| 4 | 構造設計 | 技術境界決定 | BC境界、Context Map | Ch.17-19 |
| 5 | 施工図面作成 | 詳細設計 | API、データモデル | Ch.20-22 |
| 6 | 内装・設備工事 | 動的振る舞い | イベント、ワークフロー | Ch.23-28 |
| 7 | インフラ整備 | 運用基盤 | CI/CD、監視、スケーリング | Ch.29-31 |

---

## 5. 価値駆動ZIGZAGプロセス（Axiomatic Design統合）

### 2段階ZIGZAGモデル（V5.4でのAxiomatic Design統合）

**第1段階**: 価値のZIGZAG → **第2段階**: ビジネスと設計のZIGZAG

**重要**: V5.4では、Axiomatic DesignのDesign Matrixを用いて、FR（Functional Requirement）とDP（Design Parameter）の独立性を検証します。
...
**Axiomatic Design適用**: 各ステップでFR-DPの独立性を確保
...
**Design Matrix活用**: CL3→BCマッピングで非結合（Uncoupled）または非干渉（Decoupled）設計を実現
...
## 11. Capability階層（CL1/CL2/CL3）とAxiomatic Design

### 3段階の能力分解（Design Matrixによる検証）
...
```
Phase 3 (CL3+BO)     Phase 4              Phase 5
詳細能力+BO → BC境界確定 → BC実装設計
                    ↓
              Context Map定義
```
...
## 14. DDDとの対応関係（V5.4でのAxiomatic Design統合）
...
### Parasol V5.4のマッピング（Axiomatic Design統合）
...
## 19. V5.4でのAxiomatic Design統合

### Axiomatic Designの2つの公理

| 公理 | 内容 | Parasol V5.4での適用 |
|------|------|----------------|
| **独立性の公理** | FR（機能要求）の独立性を維持 | CL→BCマッピングで結合度最小化 |
| **情報量の公理** | 情報内容を最小化 | シンプルな設計で成功確率最大化 |

### Design Matrixの3パターン

| パターン | マトリクス構造 | 評価 | 対応方法 |
|---------|------------|------|--------|
| **Uncoupled** | 対角行列 | 最良 | そのまま実装 |
| **Decoupled** | 下三角行列 | 良 | 順序保証で実装 |
| **Coupled** | 全体結合 | 不良 | 再設計必須 |

### ZIGZAGとAxiomatic Designの統合

```
ZIGZAGステップ      Axiomatic Design検証
─────────────────────────────────────
1. WHAT(何を)       → FR定義
2. HOW(どう)        → DP選択  
3. Design Matrix     → 独立性検証
4. フィードバック      → 結合解消
```
...
| 支柱 | 説明 | V5.4での強化点 |
|------|------|-------------|
| **価値駆動** | すべては価値から始まる | FRとして価値を明確化 |
| **観測可能性** | 測定できないものは管理できない | Design Matrixで可視化 |
| **段階的実現** | 一歩ずつ確実に | Decoupled設計で順序保証 |
| **構造的思考** | 表層ではなく構造を見る | FR-DP関係を明示化 |
| **継続的検証** | 常に価値を確認する | 情報量評価で改善 |
...
## 23. Parasol V5.4書籍構成（38章）

### 書籍の8部構成

| 部 | 章 | 内容 |
|-----|-----|------|
| **Part 1: 基礎** | Ch.1-5 | なぜParasol V5.4か、歴史、ZIGZAG、Axiomatic Design、統合アプローチ |
| **Part 2: 組織** | Ch.6-8 | Phase 0-1（組織アセスメント、コンテキスト確立）、業界パターン |
| **Part 3: 価値空間** | Ch.9-12 | Phase 2（価値ストリーム発見、価値ステージ、価値階層、メトリクス） |
| **Part 4: 問題空間** | Ch.13-16 | Phase 3（ZIGZAGプロセス、ケイパビリティ定義、制約管理、問題→価値変換） |
| **Part 5: アーキテクチャ・ソフトウェア・品質** | Ch.17-28 | Phase 4-6（アプリケーションアーキテクチャ、BC設計、ドメインモデル、実装品質） |
| **Part 6: 統合** | Ch.29-31 | Phase 7（プラットフォーム統合、外部システム、API契約・バージョニング） |
| **Part 7: 実践** | Ch.32-35 | エンタープライズ事例、パフォーマンスチューニング、セキュリティ、継続的改善 |
| **Part 8: 未来展望** | Ch.36-38 | 新技術統合、組織変革、未来のソフトウェア開発 |

### V5.4の特徴的な章

| 章 | タイトル | 特徴 |
|-----|----------|------|
| Ch.4 | Axiomatic Design概論 | 独立性の公理、情報量の公理 |
| Ch.5 | V5.4統合アプローチ | ZIGZAGとAxiomatic Designの融合 |
| Ch.13 | Phase 3: ZIGZAGプロセス詳細 | Design Matrixを用いたケイパビリティ分解 |
| Ch.16 | 問題から価値への変換 | FR-DPマッピングの実践 |
