...
## はじめに：歴史は繰り返さない、記録される

古代の歴史家は、出来事を年代記として記録しました。「何が起きたか」の連続が歴史を形作ります。現代のソフトウェアでも、「状態」ではなく「出来事」を記録することで、より豊かで柔軟なシステムを構築できます。

本章では、CQRS（Command Query Responsibility Segregation）とイベントソーシングという2つの強力なパターンを、Parasol V5.4の文脈で解説します。
...
CQRSとイベントソーシングは、複雑なドメインロジックと高度な読み取り要求を持つシステムに強力なソリューションを提供します。Parasol V5.4における実装の要点：

1. **明確な責任分離** - コマンドとクエリを完全に分離
2. **イベントが真実の源** - 状態ではなく出来事を記録
3. **時制の表現** - 過去、現在、未来を自然に扱える
4. **柔軟な読み取りモデル** - 用途に応じた最適化が可能
5. **完全な監査証跡** - すべての変更が追跡可能

これらのパターンは複雑性をもたらしますが、適切に適用すれば、スケーラブルで保守可能なシステムを実現できます。

### 次章への架橋

ソフトウェア設計の高度なパターンを理解したところで、次の第23章からは実装品質を確保するためのテスト戦略について解説します。

---

## 演習問題

1. 「銀行口座」ドメインをイベントソーシングで実装してください。入金、出金、送金のイベントと、残高照会のプロジェクションを含めてください。

2. 「ショッピングカート」から「注文」への変換を、CQRSパターンで設計してください。コマンド側とクエリ側のモデルを明確に分離してください。

3. 「在庫管理」システムで、複数倉庫間の在庫移動をサーガパターンで実装してください。部分的な失敗に対する補償処理も含めてください。