...
| カテゴリ | 項目 |
|---------|------|
| **分析力** | 複雑なシステムの理解、パターンの発見、問題の早期検出、多角的な視点 |
| **生成力** | ボイラープレートの自動生成、テストコードの作成、ドキュメント生成、リファクタリング提案 |
| **知識** | ベストプラクティスの適用、最新技術の理解、クロスドメイン知識、規制・標準の認識 |

**現在の限界**

| カテゴリ | 項目 |
|---------|------|
| **文脈** | プロジェクト固有の文脈、組織文化の理解、暗黙知の把握 |
| **創造性** | 真に革新的なアイデア、直感的な判断、感情的な配慮 |
| **責任** | 最終的な意思決定、倫理的判断、結果への責任 |
...
- **戦略的**: ビジネス価値の定義、ステークホルダー調整、最終的な判断
- **創造的**: 革新的なアイデア、ユーザー共感、美的センス
- **文脈的**: 組織特有の知識、政治的配慮、文化的適応

**AIの責任領域**

- **分析的**: 大量データの分析、パターン認識、矛盾の検出
- **生成的**: コード生成、テスト作成、文書化
- **支援的**: 知識の提供、選択肢の提示、品質チェック

**協働領域**

| 領域 | 人間 | AI |
|------|------|-----|
| 設計 | ビジョンとゴール設定 | 実現方法の提案 |
| 問題解決 | 問題の定義と優先順位 | 解決策の網羅的検討 |
| 品質 | 品質基準の設定 | 自動チェックと改善提案 |
...
>
> 要求：
> 全店舗の在庫をリアルタイムに把握し、店舗間で融通可能にする在庫管理システム
>
> 技術要件：
...
|----------|------|-----|
| 初期要求 | 要求を伝える | 初期提案 |
| 改善 | この部分が要件と違う | 修正案の提示 |
| 検証 | パフォーマンスが心配 | ベンチマークと改善案 |
| 最終化 | これでOK、テストも追加して | 最終実装とテスト |
...
| 時間 | 人間 | Claude |
|------|------|--------|
| 9:00 計画 | 今日は配送最適化機能を実装する | 関連する要件と制約を整理しましょう |
| 9:30 設計 | こんなアプローチを考えている | その設計の利点と課題を分析します |
| 10:00 TDD | まずテストから書きたい | ビジネス要件に基づいたテストケースを生成 |
| 11:00 実装 | 基本実装を開始 | エラーハンドリングと最適化を支援 |
| 13:00 レビュー | 午前中の実装をレビューして | セキュリティとパフォーマンスの観点から分析 |
| 14:00 統合 | 既存システムとの統合 | 統合パターンと注意点を提示 |
| 15:30 文書化 | ドキュメントを更新 | APIドキュメントと運用ガイドを生成 |
| 16:30 テスト | 統合テストを実行 | 不足しているテストケースを提案 |
| 17:00 デプロイ | ステージング環境へデプロイ | デプロイチェックリストと確認事項 |
