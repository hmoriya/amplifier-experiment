...
「良い設計」とは何か？この主観的な問いに、科学的な基準を与えるのが**Axiomatic Design（公理的設計）**です。

MIT Suh教授が提唱した2つの公理により、設計品質を測定可能にします：

| 公理 | 原則 | V5での適用 |
|------|------|-----------|
| **独立公理** | 機能要件の独立性を維持 | BC間の疎結合、循環依存の排除 |
| **情報公理** | 成功確率を最大化 | 失敗の余地を排除、実装ミスを減らす設計 |

> **情報公理の本質**：「シンプル」ではなく「**成功確率の最大化**」で理解します。
> 原典の定義：情報量 = 設計を正しく実現するために必要な「知識・判断・仕様の量」
> 情報量が少ない ＝ 迷う余地が少ない ＝ 成功確率が高い ＝ 良い設計

> 📚 **詳細な理論と実践例**: [zigzag-foundations.md](../../philosophy/zigzag-foundations.md#axiomatic-designの2つの公理)

**設計品質の判定基準（Design Matrix）**：
- **Uncoupled**（対角行列）: ✅ 理想的 - 各機能が独立
- **Decoupled**（三角行列）: ⚠️ 許容 - 順序依存あり
- **Coupled**（非対角行列）: ❌ 要改善 - 相互依存

この科学的基準により、「好みの議論」ではなく、客観的な設計レビューが可能になります。
