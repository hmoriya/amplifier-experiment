...
│   └─ value-axis                                                     │
│                                                                     │
│ 【価値実現支援】顧客価値を実現するための組織設計                    │
│   ├─ capability-axis    → 確実性・品質価値への変換                 │
│   ├─ business-unit-axis → 適合価値への変換                         │
│   ├─ platform-axis      → 選択・発見価値への変換                   │
│   ├─ fusion-axis        → シナジー価値への変換                     │
│   └─ multi-tier-vstr    → サービス体験価値への変換                 │
└─────────────────────────────────────────────────────────────────────┘
```

### 1. value-axis（価値軸）★Core VL抽出
```
顧客価値: 顧客が得る価値を直接定義
  └─ VL1→VL2→VL3→VS0-VS7 の価値体系を構築
...
### 2. capability-axis（ケイパビリティ軸）
```
顧客価値: 組織能力が顧客に届ける「確実性」「品質」「信頼」
  └─ R&D能力 → 顧客が得る「問題解決の確実性」
  └─ 品質管理能力 → 顧客が得る「裏切られない信頼」
  └─ サプライチェーン能力 → 顧客が得る「いつでも買える安心感」

組織視点: 機能横断的な能力の共通化・効率化
適合: 機能横断的な価値創出を重視する企業
例: 人材サービス、プロフェッショナルサービス
```

### 3. business-unit-axis（事業部軸）
```
顧客価値: 事業専門性が顧客に届ける「適合価値」「深い理解」
  └─ 顧客セグメント別の最適化された価値体験
  └─ 業界特化の専門知識による的確なソリューション

組織視点: 既存事業部構造を活かした漸進的変革
適合: 事業部制が明確で独立性の高い企業
例: 多角化企業、コングロマリット
```
...
```
顧客価値: ネットワークが顧客に届ける「選択肢」「発見」「マッチング」
  └─ 需要者（DS）: 最適な選択肢との出会い
  └─ 供給者（SS）: 適切な顧客へのリーチ
  └─ 両面: 信頼できる取引環境

組織視点: 共通基盤 + 事業別サービスの2層構造
...
### 5. fusion-axis（融合軸）
```
顧客価値: 統合が顧客に届ける「シナジー価値」「一貫した体験」
  └─ 共通VL1による理念レベルの統一体験
  └─ 事業別VSによる最適化されたジャーニー
  └─ 事業横断で得られる追加価値

組織視点: 共通層（Supporting）と事業層の戦略的分離
適合: 複雑な価値創造構造を持つ企業
例: 総合金融、ヘルスケア
```
...
```
顧客価値: 従業員・パートナーが顧客に届ける「サービス体験」
  └─ 従業員満足 → 顧客が得る「心のこもったサービス」
  └─ パートナー連携 → 顧客が得る「シームレスな体験」
  └─ サプライチェーン → 顧客が得る「安定供給」

組織視点: 顧客VStr + 従業員VStr + パートナーVStr の多層設計
適合: サービス品質が従業員/パートナーに依存する企業
例: サービス業、製造業、流通業
```
