...
### Phase 3で見えてくるBounded Context

Phase 3「ケイパビリティ分解」では、価値を実現するために必要な能力を洗い出します。この時、自然にBounded Contextの境界が見えてきます。

```yaml
capability: 在庫管理
  purpose: 正確な在庫数を維持し、適切な引当を行う
  boundary:
    - 含む：在庫数管理、引当、棚卸
    - 含まない：価格設定、商品情報管理
  
  # これがBounded Contextの境界になる
  context_name: 在庫コンテキスト
```

「どこに境界を引くか」という難しい問題を、「どんな能力が必要か」という具体的な問いに変換します。
...
service_design:
  ...
  aggregates:
    - 在庫:
        root: 商品在庫
        entities: [在庫履歴]
        value_objects: [在庫数, 引当情報]
        
  api_design:
    commands:
      - 在庫追加: POST /inventory/add
      - 在庫引当: POST /inventory/reserve
    queries:
      - 在庫照会: GET /inventory/{productId}
```
...
model_maturity:
  phase1:
    simple: 単一倉庫、即時引当
    good_enough_for: MVP、月商1000万円まで
...
term_definition:
  プレミアム会員:
    business_meaning: 年間10万円以上購入する優良顧客
    system_representation: Customer.tier == "PREMIUM"
    value_impact: 
      - LTV: 通常会員の3.5倍
      - 解約率: 5%（通常会員は20%）
```
...
documentation:
  ...
  location: リポジトリ内
  updates: プルリクエストで更新
  validation: Claude Codeが整合性チェック
