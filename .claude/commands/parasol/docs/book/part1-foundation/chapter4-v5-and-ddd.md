...
| CL3（オペレーション定義） | **Bounded Context** | 実装単位の定義 |
...
│ CL3: 分類なし（網羅性を重視、Bounded Contextとして定義）     │
└─────────────────────────────────────────────────────────────┘
```
...
1. **CL1**で全体を俯瞰（仮説形成）
2. **CL2**で白紙から再評価（投資判断）
3. **CL3**でBounded Contextを定義（実装境界）

この段階的アプローチにより、DDDの「正しい場所に正しく投資する」という理想を確実に実現できます。

**CL3からパラソルドメイン言語へ**

CL3で定義されたBounded Contextには、**パラソルドメイン言語**が適用されます。
...
>   - 含む：在庫数管理、引当、棚卸
>   - 含まない：価格設定、商品情報管理
> - コンテキスト名：在庫コンテキスト（これがBounded Contextの境界になる）
