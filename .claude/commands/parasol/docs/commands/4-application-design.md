...
Phase 3で定義したCL3（詳細能力）からBC境界を確定する実例：

#### 入力：Phase 3成果物（CL3一覧）

| CL3-ID | 詳細能力名 | 対応BO | 親CL2 |
|--------|-----------|--------|-------|
| CL3-001 | 酵母株研究能力 | 酵母株培養、酵母特性分析 | fermentation-research |
| CL3-002 | 発酵条件最適化能力 | 発酵パラメータ調整、品質予測 | fermentation-research |
| CL3-003 | 品質検証能力 | 成分分析、官能評価 | quality-assurance |
| CL3-004 | 製品企画能力 | 市場調査、コンセプト設計 | product-planning |
| CL3-005 | レシピ開発能力 | 配合設計、試作評価 | product-planning |

#### ステップ1：Design Matrix作成

```
CL3↔BC候補 Design Matrix
═══════════════════════════════════════════════════════════════════════════
           │ BC-Fermentation │ BC-Quality │ BC-Product │ BC-Recipe │
───────────┼─────────────────┼────────────┼────────────┼───────────┤
CL3-001    │       X         │            │            │           │  ← 単一BC ✅
CL3-002    │       X         │     △      │            │           │  ← 品質参照あり
CL3-003    │                 │     X      │            │           │  ← 単一BC ✅
...
```
分析:
- CL3-001, CL3-003, CL3-004: Uncoupled（理想）
- CL3-002: BC-Qualityを参照 → 品質データを読み取り
- CL3-005: 発酵条件と品質基準を参照 → レシピ設計に必要

結論: 順序依存があるが許容範囲
      BC-Fermentation → BC-Quality → BC-Recipe の順で設計
```

#### ステップ3：BC境界確定

| BC名 | 含まれるCL3 | 責務 | 統合パターン |
|------|------------|------|-------------|
| **BC-Fermentation** | CL3-001, CL3-002 | 発酵技術の研究・最適化 | 公開イベント発行 |
| **BC-Quality** | CL3-003 | 品質検証・基準管理 | Customer-Supplier |
| **BC-Product** | CL3-004 | 製品企画・市場分析 | Partnership |
| **BC-Recipe** | CL3-005 | レシピ開発・配合管理 | ACL（Anti-Corruption Layer） |
...
│                                                                      │
│  ┌──────────────────┐      Published Language      ┌─────────────┐ │
│  │  BC-Fermentation │ ─────────────────────────→  │  BC-Recipe  │ │
│  │  [Core]          │    （発酵パラメータ共有）    │  [Core]     │ │
│  └────────┬─────────┘                              └──────┬──────┘ │
│           │                                               │         │
│           │ Domain Events                                 │         │
│           │ （FermentationCompleted）                     │ ACL     │
│           ▼                                               ▼         │
│  ┌──────────────────┐      Customer-Supplier      ┌─────────────┐ │
│  │   BC-Quality     │ ←────────────────────────── │ BC-Product  │ │
│  │   [VCI]          │    （品質基準を提供）       │  [Core]     │ │
│  └──────────────────┘                              └─────────────┘ │
│                                                                      │
└─────────────────────────────────────────────────────────────────────┘
...
- **形状**: Decoupled（三角）
- **独立公理**: ✅ 満たす（順序依存は許容範囲）
- **情報公理**: ✅ BC数(4) / CL3数(5) = 0.8（目標範囲内）

### BC境界確定理由

**BC-Fermentation**（CL3-001 + CL3-002を統合）
- 理由: 同じドメインエキスパート（発酵研究者）が担当
- 理由: 酵母データと発酵条件は密結合
- 統合メリット: トランザクション境界が明確

**BC-Quality**（CL3-003単独）
- 理由: 独立した品質保証チームが担当
- 理由: 規制対応で監査証跡が必要
- 分離メリット: コンプライアンス要件の独立管理

**BC-Recipe**（CL3-005単独、ACL適用）
- 理由: 発酵と品質の両方を参照するが、独自の配合ロジック
- ACL適用理由: 外部データの変換・検証が必要
```

---

## 🆕 Phase 4.1 (V5.6): OS-BC整合・ステージ依存TVDCアーキテクチャ

V5.6では、Phase 3.4で定義したOperational Stage（OS）とBounded Context（BC）の整合性を確保し、
ステージ依存TVDCに基づくアーキテクチャ判断をサポートします。

### Phase 4.1の目的

1. **OS-BC整合性確保**: 各OSに対応するBCの責務を明確化
2. **ステージ依存アーキテクチャ**: Value Stageごとに最適なアーキテクチャ判断
3. **TVDC基づく投資判断**: TVDC分類に応じたBuild/Buy/Partner決定

### Step 1: OS-BC対応マッピング (`/parasol:4-os-bc-mapping`)

Phase 3.4で定義したOSとPhase 4のBCを対応付けます。
    ...
    - os:
      ...
      bc:
        ...
        responsibility: |
          ...
          サプライヤー管理とコスト最適化。
      alignment:
        status: "aligned"  # aligned | partial | misaligned
        notes: "OS責務とBC責務が完全一致"

    - os:
        id: "OS-2"
        name: "製造プロセス"
        capability_id: "CL2-1-2"
      bc:
        id: "BC-Manufacturing"
        name: "製造コンテキスト"
        responsibility: |
...
**整合性チェック**:
- `aligned`: OS責務とBC責務が完全一致
- `partial`: 一部の責務がBCにまたがる（要文書化）
- `misaligned`: 責務の不一致あり（要再設計）

### Step 2: ステージ依存TVDCアーキテクチャ判断 (`/parasol:4-stage-architecture`)

ステージ依存TVDCに基づき、各Value Stageでの最適なアーキテクチャ判断を支援します。
...
stage_architecture_decisions:
  ...
  # 対応Capabilityのステージ依存TVDC
  stage_tvdc:
    default: "VCI"
    overrides:
      VS2_VS3:
        tvdc: "Core"
        rationale: "初回購買でのスムーズな決済体験が差別化要因"

  # ステージ別アーキテクチャ判断
  architecture_decisions:
    VS2_VS3:  # 初回体験段階（Core昇格時）
      tvdc: "Core"
      recommendations:
        build_buy: "Build"
        rationale: |
          初回購買での決済体験が競争優位に直結するため、
          独自UIと最適化されたフローを内製する。
    ...
    VS4_VS7:  # 定着段階（デフォルトVCI）
      tvdc: "VCI"
      recommendations:
        build_buy: "Buy + Customize"
        rationale: |
          定着段階では決済自体は差別化にならないが、
          安定性は必須。成熟したSaaS + カスタムUI。
...
### TVDC基づくBuild/Buy/Partner判断マトリクス

| TVDC | Build/Buy | 理由 | 投資優先度 |
|------|-----------|------|-----------|
| **Core** | Build | 競争優位の源泉、独自実装で差別化 | 最高 |
| **VCI** | Buy + Customize | 必須だが差別化にならない、信頼性重視 | 高（品質重視） |
| **Supporting** | Buy or Partner | ビジネス支援、コスト効率重視 | 中 |
| **Generic** | SaaS | 汎用機能、標準ソリューション活用 | 低 |
...
│    ▼   ▼          ▼              ▼                                  │
│ ┌──────┐ ┌───────────┐ ┌───────────────┐ ┌──────────┐              │
│ │Build │ │Buy+Custom │ │Buy or Partner │ │SaaS活用  │              │
│ │独自  │ │ 品質重視  │ │ コスト効率   │ │標準利用  │              │
│ └──────┘ └───────────┘ └───────────────┘ └──────────┘              │
│                                                                      │
│  非機能要件も調整:                                                  │
│  Core → 最高レベル（99.99%、<500ms）                                │
│  VCI  → 高レベル（99.9%、<1s）                                      │
│  Supporting → 標準（99%、<3s）                                      │
│  Generic → SaaS SLA準拠                                             │
│                                                                      │
...
| 成果物 | ファイル名 | 内容 |
|--------|-----------|------|
| OS-BCマッピング | `os-bc-mapping.yaml` | OS-BC対応と整合性状態 |
| ステージ判断 | `stage-architecture-decisions.yaml` | ステージ別Build/Buy判断 |
| 非機能要件 | `stage-nfr-requirements.yaml` | ステージ別NFR設定 |
...
**検証項目**:
1. すべてのOSに対応するBCが定義されているか
2. OS責務とBC責務が整合しているか
3. ステージ依存TVDCに応じたBuild/Buy判断があるか
4. 非機能要件がTVDCレベルと整合しているか
...
入力:
- CL3詳細能力+BO: {cl3_list_from_phase3}
...
Phase 3のCL3（詳細能力）とBO（業務オペレーション）を基に、Value Stream毎にマイクロサービス境界を決定し、
...
> Phase 3のCL3（詳細能力）+BO（業務オペレーション）定義を入力として、BC境界を決定します。
