...
### ⚠️ 重要: Vision/Mission/Core ValuesとParasol価値（VL）の違い

> **よくある質問**: 「Vision、Mission、Core Valuesがあるのに、なぜパラソルで別の価値定義（VL）が必要なのか？」

この質問は本質的であり、明確に理解することがPhase 1.2の成功に不可欠です。

#### 2つの「価値」の違い

```
┌─────────────────────────────────────────────────────────────────────┐
│           企業の価値体系（Vision/Mission/Core Values）              │
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│  Vision（ビジョン）                                                 │
│  └─ 組織が目指す将来の姿・世界観                                   │
...
│  VL2（価値グループ）                                                │
│  └─ VL1を構成する価値要素群                                        │
│     例: 「高品質な製品」「健康価値」「ライフスタイル提案」         │
│                                                                     │
│  VL3（詳細価値）                                                    │
│  └─ 測定・実装可能な具体的価値                                     │
│     例: 「糖質30%オフビール」「機能性表示食品」「時短レシピ」     │
│                                                                     │
│  ★ これらは「顧客が何を受け取るか」を定義                          │
│  ★ WHAT（何を届けるか）を規定                                      │
│  ★ 市場・顧客理解の深化により進化（深化）                          │
...
| 概念 | 企業価値体系 | Parasol価値（VL） |
|------|-------------|-------------------|
| **定義** | 組織の原則・文化 | 顧客が受け取る価値 |
| **例** | 「挑戦と革新」（Core Value） | 「糖質オフで罪悪感なく楽しめるビール」（VL3） |
| **変化** | めったに変わらない | 市場理解により深化・進化 |
| **測定** | 文化浸透度（間接） | 売上・NPS・利用率（直接） |
| **対象** | 従業員・組織 | 顧客・市場 |

#### なぜ区別が重要か

1. **測定可能性**: Core Valuesは「誠実であるか」を数値化しにくいが、VL3は「糖質オフ製品売上」として測定可能

2. **設計への落とし込み**: 「挑戦と革新」からAPIは設計できないが、「糖質オフ製品のパーソナライズ提案」からは設計可能

3. **トレーサビリティ**: Parasol VLは全てのフェーズを通じて追跡可能
   ```
   VL3 → VMS → VS → CL → BC → UseCase → API/DB → View
   ```

4. **進化のメカニズム**: Vision/Missionは安定させつつ、VLは顧客理解の深化により「深化」していく

#### Phase 1.2での扱い

```yaml
# organization-analysis.md で既に収集済み
enterprise_values:
  vision: "組織のビジョン"       # 参照情報として記録
  mission: "組織のミッション"    # 参照情報として記録
  core_values: ["価値1", "価値2"] # 参照情報として記録

# value-hierarchy.md で新規定義
parasol_values:
  VL1: "顧客最上位価値"          # ← これを設計対象とする
  VL2: ["価値グループA", "価値グループB"]
  VL3: ["詳細価値1", "詳細価値2", "詳細価値3"]
```

**重要**: Vision/Mission/Core Valuesは「文脈」として尊重しつつ、Parasol VLは「顧客に届ける価値」として別途定義・測定・トレースします。
