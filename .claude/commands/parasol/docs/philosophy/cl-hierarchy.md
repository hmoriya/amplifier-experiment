...
CL1 (Value Stream Level) ──┬── Core/Supporting/Generic: 傾向的分類（参考情報）
        ...
        ↓ 1:N
CL2 (Capability Level) ────┬── TVDC 4分類: Core/VCI/Supporting/Generic（★正式分類）
        │                  ├── 差別化度: Phase 3で評価
...
Service Boundary ──────────┬── サービス境界分析（CL2→Service対応）
        ...
        ↓ N:M
                           ...
                           【Phase 5: ソフトウェア設計】
BC (Bounded Context) ──────┬── TVDC分類継承（親CL2から）
        │                  └── 内容：Domain Model(SSOT) + DB + API
        ↓ 1:N
...
V5.5では、従来の3分類に**VCI（Value-Critical Infrastructure）**を追加した4分類を採用します。
...
classification:
  ...
  value_necessity_source: "Phase 2 VL3から継承"  # V5.5新規
  ...
  criteria:
    Core:
      ...
      characteristics:
        ...
        - "独自のノウハウ・ロジックを含む"
        - "専用マイクロサービス候補"
      decision: "内製・高投資（差別化機能開発）"

    VCI:  # V5.5新規
      definition: "差別化しないが、価値提供に不可欠なCapability"
      differentiation: "★（低）"
      value_necessity: "★★★（高）"
      characteristics:
        - "障害時の価値喪失が甚大"
        ...
        - "高品質・高可用性が必須"
      examples:
        - "決済処理（決済方式で差別化しないが、失敗は致命的）"
        - "認証基盤（認証で競争しないが、セキュリティ障害は壊滅的）"
        - "データ配信（配信方法で競争しないが、遅延は顧客体験を破壊）"
      decision: "標準技術採用・信頼性投資優先"
...
### なぜ同じBOがWHATにもHOWにもなるのか？

「CL3のHOWであるBOが、Actor UseCaseのWHATになるのは矛盾ではないか？」という疑問が生じることがあります。

**回答**: 矛盾ではなく、**視点（レベル）によってWHAT/HOWが相対的に変わる**階層構造です。

### 建築の例で理解する

```
施主の視点:   WHAT「家が欲しい」      → HOW「設計図」
建築士の視点: WHAT「設計図通りに」    → HOW「施工計画」
職人の視点:   WHAT「施工計画通りに」  → HOW「実際の作業」
...
┌─────────────────────────────────────────────────────────────────┐
│【ビジネスレベル】業務担当者の視点                                │
│  WHAT: CL3（詳細能力）          HOW: BO（業務オペレーション）    │
│  「何ができるか」                「どう業務遂行するか」          │
└─────────────────────────────────────────────────────────────────┘
                                    │
                                    │ BOが次のレベルのWHATになる
...
┌─────────────────────────────────────────────────────────────────┐
│【システムレベル】PO・開発者の視点                                │
│  WHAT: BO（何を実現すべきか）   HOW: Actor UseCase + View        │
│  「システム要件」                「どう実装するか」              │
└─────────────────────────────────────────────────────────────────┘
                                    │
                                    │ Actor UseCaseが次のレベルのWHATになる
                                    ▼
┌─────────────────────────────────────────────────────────────────┐
│【実装レベル】開発者の視点                                        │
│  WHAT: Actor UseCase            HOW: API + DB + Code             │
│  「何の機能か」                  「どう実装するか」              │
└─────────────────────────────────────────────────────────────────┘
```
...
| レベル | WHAT（何を） | HOW（どう） | 対象者 |
|--------|-------------|-------------|--------|
| ビジネス | CL3（能力） | BO（業務活動） | 業務担当者 |
| システム | BO（要件） | Actor UseCase | PO・開発者 |
| 実装 | Actor UseCase | API/DB/Code | 開発者 |

### BIZBOKでも同じ構造

```
Capability（WHAT）→ Process Activity（HOW）
                          │
                          ▼ 視点を変えると
                    Process Activity（WHAT）→ Subprocess（HOW）
                                                    │
                                                    ▼
                                              Subprocess（WHAT）→ Task（HOW）
```

### 結論

> **WHAT/HOWは絶対的なものではなく、どのレベルから見るかで相対的に決まります。**
> **上位レベルのHOW（どうやって）が、下位レベルのWHAT（何を）になる。**
> **これは矛盾ではなく、設計の階層化（ZIGZAGパターン）の本質です。**
...
Parasolにおける設計の核心は、以下の2つの分解プロセスです。
...
### 最重要分解2: ケイパビリティ → UseCase → API/DB

```
┌─────────────────────────────────────────────────────────────────────────────┐
│        最重要分解2: ケイパビリティ → UseCase → API → DB                     │
│                  （Phase 3-5: ビジネス能力の実装設計）                       │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│  【Phase 3: ケイパビリティ分解】                                            │
│                                                                             │
...
│   │  例: VS2「製品開発」                                                    │
│   │  ├─ Core/Supporting/Generic: 傾向的分類（参考）                        │
│   │  └─ 経営判断: どのVSに投資するか                                       │
│   │                                                                         │
│   └─→ CL2 Capability（ケイパビリティ）── 1:N関係                           │
│        │  例: 「発酵研究」「商品企画」「品質保証」                          │
│        │  ├─ Core/Supporting/Generic: ★正式分類（投資判断）               │
│        │  └─ 事業判断: 内製/外注の意思決定                                 │
│        │                                                                    │
│        └─→ CL3 Sub-capability（詳細能力）── 1:N関係                       │
│             │  例: 「酵母研究能力」「発酵最適化能力」「品質検証能力」      │
│             │  ├─ 分類なし（網羅性が目的）                                 │
...
│  │【Phase 4: サービス境界分析】                                          │  │
│  │                                                                       │  │
│  │  CL2 Capability ─────→ Service Boundary（サービス境界）              │  │
│  │       │                    └─→ Context Map（BC間関係）               │  │
│  │       │                         ・Shared Kernel                      │  │
│  │       │                         ・Customer-Supplier                  │  │
...
│  【Phase 5: ソフトウェア設計】                                              │
│             │                                                               │
│             └─→ BC（Bounded Context）── 分類はCL2から継承                  │
│                  │                                                          │
│                  ├─→ Domain Model（SSOT: 単一の真実の源）                  │
│                  │    └─ domain-language.md                                │
│                  │       ├─ 用語定義（Glossary）                           │
│                  │       ├─ Aggregate Root                                 │
│                  │       ├─ Entity / Value Object                          │
...
│                  │       └─ Relationships                                  │
│                  │                                                          │
│                  └─→ Actor UseCase（シングルアクター原則）── 1:N関係       │
│                       │  例: 「研究者が酵母株を登録する」                  │
│                       │  ├─ 1 Actor UseCase = 1 API Endpoint              │
│                       │  └─ Agile User Storyに対応（INVEST原則）          │
│                       │                                                     │
│                       └─→ View（画面実装）── 1:N関係                       │
│                            例: 「酵母株登録画面」「検索結果画面」           │
...
│  ├─ CL2ケイパビリティ一覧（正式分類）                                      │
│  ├─ CL3詳細能力+BO一覧                                                     │
│  ├─ Service Boundary + Context Map                                         │
│  ├─ BC定義（Domain Model + API + DB）                                      │
│  ├─ Actor UseCase一覧（User Story形式）                                    │
│  └─ View一覧                                                               │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
```
...
│   VL3（詳細価値）─────────────┐                                            │
│    ↓                          │                                            │
│   VMS（価値マイルストーン）    │ 価値トレーサビリティ                      │
│    ↓                          │                                            │
│   VS（バリューストリーム）─────┼────→ CL1（活動領域）                      │
│                               │        ↓                                   │
│                               │       CL2（ケイパビリティ）                │
│                               │        ↓                                   │
│                               │       CL3（詳細能力）→BO                   │
...
│                                                                             │
│  ┌───────────────────────────────────────────────────────────────────────┐ │
│  │ 価値トレーサビリティ: すべての実装が価値に紐付く                     │ │
│  │                                                                       │ │
│  │ VL3「機能性食品による栄養補給」                                      │ │
│  │  └─→ VS2「製品開発」                                                 │ │
│  │       └─→ CL2「発酵研究」[Core]                                      │ │
│  │            └─→ CL3「酵母株培養」                                     │ │
│  │                 └─→ BC「fermentation-research」                      │ │
│  │                      └─→ Actor UseCase「研究者が酵母株を登録する」   │ │
│  │                           └─→ View「酵母株登録画面」                 │ │
│  └───────────────────────────────────────────────────────────────────────┘ │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
