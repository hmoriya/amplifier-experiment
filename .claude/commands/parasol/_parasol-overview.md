...
**バージョン**: V5.11（分析フロー全体像を追加：Phase 2-5のビジネス分解+技術設計統合図）

本ドキュメントは Parasol V5 フレームワークの全体像を視覚的に示します。

## 設計哲学

Parasolは、長年の実践経験から導き出された「**保守性と変更容易性**」を最優先に設計されています。見た目の素晴らしさより、修正のしやすさこそが、システムの長期的な成功を決定します。
...
┃  ║  │                  │              │ - Bounded Context    │      ║      ┃
┃  ║  │ CL1: 活動領域(傾向)│             │ - Context Map        │      ║      ┃
┃  ║  │ CL2: ケイパビリティ(正式)│        │ - 【実装UC開始】     │      ║      ┃
┃  ║  │ CL3(≈BizOp)        │          └──────────────────────┘      ║      ┃
┃  ║  └──────────────────┘                                             ║      ┃
┃  ╚══════════════════════════════════════════════════════════════════╝      ┃
┃              ↓                              ↓                               ┃
...
### VStr（Value Stream）の判定基準

**重要**: 何かをValue Streamとして定義する前に、以下の基準を確認してください。

| 基準 | Yes → VStr | No → 別概念 |
|------|-----------|------------|
| **価値の受け手は明確か？** | 顧客、従業員、パートナー | 「事業部」「組織」「会社」 |
| **状態遷移が設計可能か？** | VS0→VS7のジャーニー | 関係性、基盤機能、連携 |
| **時間文脈があるか？** | 顧客ジャーニーの時間軸 | 構造的関係、常時稼働 |

#### シナジー ≠ Value Stream

コングロマリットや多角化企業で「シナジー」をVStrとして定義しようとする誤りがよく見られます：

```
❌ 誤解: 「シナジーVStr」で事業間連携を定義
   → 「事業部」は顧客ではなく、状態遷移（VS0→VS7）が成立しない

✅ 正解: シナジーは以下の3概念に分解される
   1️⃣ BC間関係（Context Map）: Partnership、Customer-Supplier
   2️⃣ Supporting Layer: 共通R&D、品質保証基盤、物流基盤
   3️⃣ Generic Layer: 統合調達、顧客データ基盤、ブランド資産
```

#### 共通価値の扱い

企業理念・共通価値（例：「水と生きる」「食と健康」）はVStrではなく**設計原則**として扱います：

```
❌ 誤: 「水と生きる VStr」を定義
✅ 正: 設計原則として全VStrに貫通させる

VS0: 潜在ニーズ発見 ←「水と生きる」視点で発見
VS2: 製品開発      ←「水と生きる」基準で開発
VS5: 価値実感      ←「水と生きる」体験を提供
```
...
┃                              ┃                                             ┃
┃   Phase 3 CL3(≈BizOp)        ┃    Phase 4以降: BC内Use Case               ┃
┃                              ┃                                             ┃
┣━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━╋━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┫
┃                              ┃                                             ┃
┃  【目的】                    ┃  【目的】                                   ┃
...
│   └── {vs-slug}/                          #   VS毎のディレクトリ
│       ├── cl1-activity-identification.md  #   CL1: 傾向識別（参考情報）
...
| Activity Area (AA) | 3 CL1 | - | Core/Supporting/Generic傾向（参考） |
| Capability (Cap) | 3 CL2 | ≈ Subdomain | サービス候補、チーム境界、★正式分類 |
...
│  └── ビジネス観点での分解単位                                       │
│          │                                                          │
│          │ CL3内のActor UC/Viewを技術境界でグルーピング             │
│          ▼                                                          │
│  Bounded Context (BC) = 技術境界                                    │
│  ├── Actor UC/View をまとめる技術的な境界                           │
│  ├── API群・DB・デプロイ単位                                        │
│  └── マイクロサービスの実装単位                                     │
│                                                                     │
│  【重要】BCは分解の「段階」ではなく、技術実装時の「グルーピング」   │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```
...
| 観点 | CL3（ビジネスオペレーション） | BC（技術境界） |
|------|------------------------------|----------------|
| **定義** | 業務活動（複数アクター協調） | Actor UC/Viewの技術的グルーピング |
| **比喩** | 仕事のプロジェクト | プロジェクトを実現するシステム |
| **粒度** | Epic/Feature単位 | マイクロサービス単位 |
| **決定基準** | 業務目的の完結単位 | 技術的凝集性・デプロイ単位 |
| **分解フロー** | 7段階目（ビジネス分解） | 分解段階ではない（技術グルーピング） |
| **Phase** | Phase 3で定義 | Phase 4で技術境界として確定 |
...
### CL2・CL3・BC の関係整理（V5.9 修正版）

```
┌─────────────────────────────────────────────────────────────────────────────┐
│              CL2・CL3・BC の関係（BCは技術グルーピング）                      │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│  【ビジネス分解フロー】VStr → VS → CL1 → CL2 → CL3 → Actor UC → View       │
│                                                                             │
│  ┌─────────────────────────────────────────────────────────────────────┐   │
│  │  CL2: ケイパビリティ（チーム境界）                                   │   │
│  │  ─────────────────────────────────────────────────────────────────  │   │
...
│  │                                                                      │   │
│  │      ┌─────────────────────────────────────────────────────────┐    │   │
│  │      │  BC: 技術境界（Actor UC/Viewをグルーピング）             │    │   │
│  │      │  ───────────────────────────────────────────────────    │    │   │
│  │      │  ・CL3内のActor UC/Viewをまとめる技術的な境界           │    │   │
│  │      │  ・API群・DB・デプロイ単位                               │    │   │
│  │      │  ・マイクロサービスの実装単位                            │    │   │
│  │      │                                                          │    │   │
│  │      │  Actor UseCase → View                                    │    │   │
...
│  └─────────────────────────────────────────────────────────────────────┘   │
│                                                                             │
│  【重要】BCは分解の「段階」ではなく、技術実装時の「グルーピング境界」       │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
```
...
| 概念 | 境界の種類 | 役割 | 分解フローでの位置 |
|------|-----------|------|-------------------|
| **CL2** | 組織境界（人） | チーム編成・責任範囲 | 4番目（ビジネス分解） |
| **CL3** | 業務境界（活動） | 業務オペレーション | 5番目（ビジネス分解） |
| **BC** | 技術境界（システム） | Actor UC/Viewのグルーピング | **分解段階ではない** |

### 具体例：CL2・CL3・BC の関係

```
CL2: 受注オーケストレーションチーム（チーム境界）
│
...
│                                                                             │
│    CL2（チーム境界）                                                        │
│    └── CL3（ビジネスオペレーション）← 業務活動単位                          │
│        └── Actor UseCase（ユーザ操作）← User Story                         │
│            └── View（UI画面）← Task                                        │
│                                                                             │
│    BC（技術境界）= Actor UC/Viewをグルーピングする技術的な境界              │
│    ※ BCは分解の「段階」ではなく、技術実装時の「グルーピング」              │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
```
...
パラソルの設計は「価値 → 時間 → 領域 → 能力 → 活動 → 操作 → 画面」という7段階で分解されます。
BCは分解の「段階」ではなく、Actor UC/Viewを技術的にグルーピングする「境界」です。
...
┃   │  Core / Supporting / Generic の傾向識別（参考情報）                  │    ┃
┃   │  例：A1基盤技術研究(Core傾向)、S1物流(Supporting傾向)                │    ┃
┃   │  【役割】投資方向性の初期判断（正式分類はCL2で行う）                  │    ┃
┃   └─────────────────────────────────────────────────────────────────────┘    ┃
┃                              │                                                ┃
┃                              │ 領域内でケイパビリティ分離                     ┃
┃                              ▼                                                ┃
┃   ┌─────────────────────────────────────────────────────────────────────┐    ┃
┃   │  CL2: Capability（ケイパビリティ）★正式分類の場所                    │    ┃
┃   │  チーム境界・サービス候補・Core/Supporting/Generic正式分類            │    ┃
┃   │  例：発酵研究、素材研究、商品提案、受注オーケストレーション             │    ┃
┃   │  【役割】チーム編成・サービス候補の特定                               │    ┃
┃   └─────────────────────────────────────────────────────────────────────┘    ┃
┃                              │                                                ┃
┃                              │ 活動単位で分解                                 ┃
...
┃   │                                                                      │    ┃
┃   │  ┌────────────────────────────────────────────────────────────┐     │    ┃
┃   │  │  BC: Bounded Context（技術境界）                           │     │    ┃
┃   │  │  ────────────────────────────────────────────────────────  │     │    ┃
┃   │  │  Actor UC/Viewを技術的にグルーピングする境界                │     │    ┃
┃   │  │  ※分解の「段階」ではなく「グルーピング」                   │     │    ┃
┃   │  │  例：order-management-bc = API群 + DB + デプロイ単位       │     │    ┃
┃   │  └────────────────────────────────────────────────────────────┘     │    ┃
┃   │                                                                      │    ┃
┃   └─────────────────────────────────────────────────────────────────────┘    ┃
┃                              │                                                ┃
...
### 分析フロー全体像（ビジネス分解 + 技術設計）

ビジネス分解（Phase 3）と技術設計（Phase 4-5）を統合した完全なフロー：
...
┃      ▼                                                                        ┃
┃    VS0 → VS1 → VS2 → VS3 → VS4 → VS5 → VS6 → VS7                             ┃
┃    認知   興味   検討   購入   受取   使用   支持   推奨                       ┃
┃      │                                                                        ┃
┃      │ ★ VS Swimlane必須（後方参照のみ、データ重複なし）                      ┃
┃      ▼                                                                        ┃
┃                                                                               ┃
┃  ═══════════════════════════════════════════════════════════════════════════  ┃
...
┃    ┌─────────────────────────────────────────────────────────────────────┐    ┃
┃    │  Core傾向         │  Supporting傾向     │  Generic傾向             │    ┃
┃    │  競争優位・投資大  │  必要だが差別化せず  │  汎用・外部調達可        │    ┃
┃    └─────────────────────────────────────────────────────────────────────┘    ┃
┃      │                                                                        ┃
┃      │ 領域内でケイパビリティ分離                                             ┃
┃      ▼                                                                        ┃
┃    CL2: Capability（チーム境界）★正式なCore/Supporting/Generic分類           ┃
┃    ┌─────────────────────────────────────────────────────────────────────┐    ┃
┃    │  1チーム = 1 CL2 = 1 BC候補（Conway's Law）+ 正式分類               │    ┃
┃    │  例：受注オーケストレーション、在庫インテリジェンス、配送オーケストレーション│┃
┃    └─────────────────────────────────────────────────────────────────────┘    ┃
┃      │                                                                        ┃
┃      │ 活動単位で分解                                                         ┃
┃      ▼                                                                        ┃
┃    CL3: Business Operation（Epic/Feature）                                    ┃
┃    ┌─────────────────────────────────────────────────────────────────────┐    ┃
┃    │  複数アクター協調の業務活動                                          │    ┃
┃    │  例：見積作成、受注処理、出荷指示                                    │    ┃
┃    └─────────────────────────────────────────────────────────────────────┘    ┃
...
┃      │ 単一ユーザ操作に分解                                                   ┃
┃      ▼                                                                        ┃
┃    Actor UseCase（User Story）→ View（Task）                                  ┃
┃                                                                               ┃
┃  ═══════════════════════════════════════════════════════════════════════════  ┃
...
┃    ┌─────────────────────────────────────────────────────────────────────┐    ┃
┃    │  BC間の関係パターンを定義                                            │    ┃
┃    │  ・Partnership / Customer-Supplier / ACL / Conformist 等             │    ┃
┃    │  ・Upstream/Downstream の依存方向を明示                              │    ┃
┃    │  成果物: context-map.md                                              │    ┃
┃    └─────────────────────────────────────────────────────────────────────┘    ┃
┃      │                                                                        ┃
...
┃    DB設計（BC単位で分離）                                                     ┃
┃    ┌─────────────────────────────────────────────────────────────────────┐    ┃
┃    │  Phase 1: 論理分離（共有DB + スキーマ分離）                          │    ┃
┃    │  Phase 2: 物理分離（BC単位で独立DB）← 必要に応じて                   │    ┃
┃    │  ★ VS時間軸により後方参照のみ → データ重複なし                      │    ┃
┃    └─────────────────────────────────────────────────────────────────────┘    ┃
┃      │                                                                        ┃
┃      ▼                                                                        ┃
┃    API設計・ドメイン言語・実装詳細                                            ┃
...
| 番号 | レベル | 分解軸 | 定義 | アジャイル対応 | Phase |
|------|--------|--------|------|---------------|-------|
| 1 | **VStr** | 価値の流れ | 顧客価値の全体像 | - | 2 |
| 2 | **VS** | 時間 | 顧客状態遷移 | - | 2 |
| 3 | **CL1** | 活動領域 | Core/Supporting/Generic傾向（参考） | Portfolio | 3 |
| 4 | **CL2** | 能力単位 | ケイパビリティ + ★正式分類 | Product/チーム | 3 |
| 5 | **CL3** | 業務活動 | ビジネスオペレーション | Epic/Feature | 3 |
| 6 | **Actor UC** | 単一操作 | ユーザの完結操作 | User Story | 4-5 |
| 7 | **View** | UI画面 | 操作インターフェース | Task | 5 |

**BC（Bounded Context）の位置づけ**:
- BCは分解の「段階」ではなく、Actor UC/Viewをグルーピングする「技術境界」
- Phase 4で技術的な境界として定義
- BC:サービス = 1:1（理想形）、N:1（統合型）、1:N（分割型）のいずれも可

**境界の関係**:
- CL2（チーム境界）: 1チーム → N個のCL3を担当
- CL3（業務境界）: 1 CL3 → N個のActor UC/View
- BC（技術境界）: CL3内のActor UC/Viewを技術的にグルーピング
...
└─ VS3: 販売段階
   ...
   └─ CL1: A3販売【Core Domain】
      │
      ┌──────────────────────────────────────────────────────────────────┐
      │ CL2: 受注オーケストレーション = BC: order-orchestration-bc        │
      │ （チーム境界 ≒ サービス境界：Conway's Law）                       │
      │                                                                  │
      │  ├─ CL3: 見積作成オペレーション ← Epic（複数アクター協調）        │
      │  │  │                                                            │
      │  │  ├─► Actor UseCase: 見積書を作成する ← User Story             │
      ...
      │                                                                  │
      │  ─────────────────────────────────────────────────────────────   │
      │  技術実装: API群 + DBスキーマ + コード（BC内で閉じる）            │
      └──────────────────────────────────────────────────────────────────┘
```

**ポイント**: BC（Bounded Context）はCL2レベル（チーム境界）と一致する設計判断。
1つのチームが1つのBCを所有し、その中に複数のCL3（オペレーション）を含む。
...
/parasol:3-capabilities cl1       # CL1: 活動領域識別（傾向的分類）
/parasol:3-capabilities cl2 VS2   # CL2: Capability設計
/parasol:3-capabilities cl3 {cap} # CL3: 詳細分解（≈Business Operation）
