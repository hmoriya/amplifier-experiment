...
@click.option('--from-contract', '-c', type=click.Path(exists=True), 
              help='Path to module contract file')
@click.option('--variants', '-v', multiple=True,
              help='Variants to generate (e.g., api:graphql, database:postgresql)')
@click.option('--output-dir', '-o', type=click.Path(),
              default='./modules', help='Output directory')
@click.option('--dry-run', is_flag=True, help='Show what would be generated without creating files')
async def generate_module(module_name: str, from_contract: str, variants: List[str], 
...
@click.argument('module_name')
@click.option('--contract', '-c', type=click.Path(exists=True),
              help='Path to updated contract file')
@click.option('--preserve', '-p', multiple=True, 
...
async def regenerate_module(module_name: str, contract: str, preserve: List[str], 
                           show_diff: bool, dry_run: bool):
...
@click.argument('module_name')
@click.option('--contract-change', '-c', type=click.Path(exists=True),
              help='Path to new contract file for impact analysis')
@click.option('--show', '-s', multiple=True,
...
def analyze_impact(module_name: str, contract_change: str, show: List[str]):
    ...
    if 'affected-modules' in show:
        ...
        for change in sorted(changes, key=lambda c: c.impact_score, reverse=True):
            impact_level = "🔴 High" if change.impact_score >= 8 else "🟡 Medium" if change.impact_score >= 5 else "🟢 Low"
...
@click.option('--from-composition', '-c', type=click.Path(exists=True),
              help='Path to system composition file')
@click.option('--environment', '-e', default='development',
              help='Target environment')
@click.option('--output-dir', '-o', type=click.Path(), default='./systems',
              help='Output directory')
async def compose_system(system_name: str, from_composition: str, 
                        environment: str, output_dir: str):
...
@click.option('--auto-regenerate', is_flag=True, 
              help='Automatically regenerate on contract changes')
@click.option('--preserve', '-p', multiple=True,
              type=click.Choice(['business-logic', 'test-data', 'configuration']),
              default=['business-logic', 'test-data', 'configuration'],
