...
class HandlebarsTemplate(CodeTemplate):
    ...
    def __init__(self, template_path: Path, output_pattern: str):
        ...
        self.env = jinja2.Environment(
            loader=jinja2.FileSystemLoader(template_path.parent), trim_blocks=True, lstrip_blocks=True
        )
    ...
    def get_output_path(self, context: GenerationContext) -> Path:
        ...
        output_path = self.output_pattern.format(module_name=context.module_name, variant=context.variant)
...
class ContractChangeDetector:
    ...
    def detect_changes(self, old_contract: Dict, new_contract: Dict) -> List[ContractChange]:
        ...
        # Domain changes
        changes.extend(self._detect_domain_changes(old_contract.get("domain", {}), new_contract.get("domain", {})))

        # API changes
        changes.extend(
            self._detect_api_changes(
                old_contract.get("adapters", {}).get("primary", {}), new_contract.get("adapters", {}).get("primary", {})
            )
        )

        # Use case changes
        changes.extend(
            self._detect_use_case_changes(old_contract.get("use_cases", {}), new_contract.get("use_cases", {}))
        )
    ...
    def _detect_domain_changes(self, old_domain: Dict, new_domain: Dict) -> List[ContractChange]:
        ...
        for name in set(old_aggregates.keys()) | set(new_aggregates.keys()):
            if name not in old_aggregates:
                changes.append(
                    ContractChange(
                        type=ChangeType.ADDED, path=f"domain.aggregates.{name}", new_value=new_aggregates[name]
                    )
                )
            elif name not in new_aggregates:
                changes.append(
                    ContractChange(
                        type=ChangeType.REMOVED, path=f"domain.aggregates.{name}", old_value=old_aggregates[name]
                    )
                )
    ...
    def _detect_use_case_changes(self, old_use_cases: Dict, new_use_cases: Dict) -> List[ContractChange]:
        ...
        for name in set(old_use_cases.keys()) | set(new_use_cases.keys()):
            if name not in old_use_cases:
                changes.append(
                    ContractChange(type=ChangeType.ADDED, path=f"use_cases.{name}", new_value=new_use_cases[name])
                )
            elif name not in new_use_cases:
                changes.append(
                    ContractChange(type=ChangeType.REMOVED, path=f"use_cases.{name}", old_value=old_use_cases[name])
                )
...
class CodePreserver:
    ...
    def extract_preserved_code(self, module_path: Path, preserve_options: Dict[str, bool]) -> PreservedCode:
        preserved = PreservedCode(business_logic={}, test_data={}, configuration={}, custom_implementations={})
    ...
    def _extract_configuration(self, module_path: Path) -> Dict[str, Any]:
        ...
        config_files = [module_path / "config.json", module_path / "config.yaml", module_path / ".env.local"]
...
class RegenerationEngine:
    ...
    async def regenerate_module(
        ...
        preserve_options = preserve_options or {"business_logic": True, "test_data": True, "configuration": True}
        ...
        for variant_type, variant_options in variants.items():
            for variant_name in variant_options:
                variant_files = await self._generate_variant(
                    module_name, new_contract, f"{variant_type}_{variant_name}", preserved_code
                )
    ...
    async def _generate_variant(
        self, module_name: str, contract: Dict[str, Any], variant: str, preserved_code: Optional[PreservedCode]
    ) -> Dict[str, str]:
        """Generate code for a specific variant."""

        context = GenerationContext(
            module_name=module_name, contract=contract, variant=variant, preserved_code=preserved_code
        )
...
class ModuleCompositionEngine:
    ...
    async def compose_system(self, composition_config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for module_name, module_config in self._sort_by_dependencies(modules):
            ...
            result = await self.regeneration_engine.regenerate_module(
                module_name, contract, preserve_options=module_config.get("preserve_options")
            )
    ...
    def _generate_integration_layer(
        self, integration_config: Dict[str, Any], modules: Dict[str, Any]
    ) -> Dict[str, str]:
        ...
        if "api_gateway" in integration_config:
            gateway_config = self._generate_api_gateway_config(integration_config["api_gateway"], modules)
        ...
        if "event_bus" in integration_config:
            event_config = self._generate_event_bus_config(integration_config["event_bus"], modules)
            integration_files["event-bus.yaml"] = yaml.dump(event_config)

        return integration_files

    def _generate_api_gateway_config(self, gateway_config: Dict[str, Any], modules: Dict[str, Any]) -> Dict[str, Any]:
        """Generate API gateway configuration."""
        return {
            "type": gateway_config.get("type", "graphql-federation"),
            "services": [
                {"name": module_name, "url": f"http://{module_name}:4000/graphql"}
                for module_name in gateway_config.get("services", [])
            ],
        }

    def _generate_event_bus_config(self, event_config: Dict[str, Any], modules: Dict[str, Any]) -> Dict[str, Any]:
        """Generate event bus configuration."""
        return {
