...
class CapabilityMapper:
    ...
    def save_map(self):
        ...
        with open(map_file, 'w', encoding='utf-8') as f:
            yaml.dump(
                self.current_map.model_dump(mode='json'),
                f,
                allow_unicode=True,
                sort_keys=False
            )
    ...
    def _define_operations(self, capability: Capability):
        ...
        for i in range(num_ops):
            op = Operation(
                ...
                automation_level=Prompt.ask("自動化レベル", default="manual", 
    ...
    def assess_capability(self, capability_id: Optional[str] = None):
...
@click.pass_obj
def assess(mapper: CapabilityMapper, capability: Optional[str]):
    """Assess capability maturity"""
...
@click.option('--pattern', '-p', type=click.Choice(['entity-service', 'task-service', 'utility-service']), 
